const { Tagline, TajaSamachar } = require('../models/Tagline.models');;
...
const getTajaSamachar = async (req, res) => {
  try {
...
const DeleteTajaSamachar = async (req, res) => {
  const Id = req.params.id;

  try {
    const result = await TajaSamachar.deleteOne({ _id: Id });
    if (result.deletedCount === 0) {
      return res.status(404).json({ error: "Product not found" });
    }
    res.json({ message: "Product deleted successfully" });
